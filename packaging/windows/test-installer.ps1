...
$customInstallDir = Join-Path $env:SystemDrive 'Average Assistant Test Install'
...
if ((Test-Path -LiteralPath $installDir) -or (Test-Path -LiteralPath $customInstallDir) -or (Test-Path -LiteralPath $dataDir)) {
...
function Assert-Installed([string]$ExpectedDir) {
    ...
    foreach ($file in @('USBC-Average-Lookup.exe', '_internal\Average-Assistant-SignIn.exe', 'unins000.exe')) {
        if (!(Test-Path -LiteralPath (Join-Path $ExpectedDir $file))) { throw "Missing installed file: $file" }
    }
    $shell = New-Object -ComObject WScript.Shell
    foreach ($link in @($shortcut, $desktopShortcut)) {
        if (!(Test-Path -LiteralPath $link)) { throw "Missing shortcut: $link" }
        if ($shell.CreateShortcut($link).TargetPath -ne (Join-Path $ExpectedDir 'USBC-Average-Lookup.exe')) {
            throw 'Shortcut points to the wrong application.'
        }
    }
    if (!(Test-Path $uninstallKey)) { throw 'Missing Windows uninstall entry.' }
}
function Assert-Uninstalled([string]$PreviousDir) {
    for ($attempt = 0; $attempt -lt 50 -and (Test-Path -LiteralPath $shortcut); $attempt++) {
        Start-Sleep -Milliseconds 200
    }
    Assert-DataPreserved
    foreach ($path in @($shortcut, $desktopShortcut, (Join-Path $PreviousDir 'USBC-Average-Lookup.exe'))) {
...
foreach ($pass in 1..2) {
    Run-Installer $installerPath @('/VERYSILENT', '/SUPPRESSMSGBOXES', '/NORESTART', '/CURRENTUSER', '/TASKS=desktopicon')
    Assert-Installed $installDir
}
Run-Installer (Join-Path $installDir 'unins000.exe') @('/VERYSILENT', '/SUPPRESSMSGBOXES', '/NORESTART')
Assert-Uninstalled $installDir

# Verify that the standard destination page can install in a folder directly on C:.
Run-Installer $installerPath @('/VERYSILENT', '/SUPPRESSMSGBOXES', '/NORESTART', '/CURRENTUSER', "/DIR=$customInstallDir", '/TASKS=desktopicon')
Assert-Installed $customInstallDir
Run-Installer (Join-Path $customInstallDir 'unins000.exe') @('/VERYSILENT', '/SUPPRESSMSGBOXES', '/NORESTART')
Assert-Uninstalled $customInstallDir
Write-Output 'PASS: default and C-drive custom installs, reinstall, shortcuts, uninstall, and saved database preservation.'
